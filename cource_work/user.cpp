    ...
    , ui(new Ui::User)
{
    ...
    ui->pushButton_2->setStyleSheet("background-color:#e8560d; color: #0c0c0c; padding:10px; font: 16pt");
}
...
void User::on_programms_itemClicked(QTableWidgetItem* item)
{
    ...
    while (programm->getQry()->next()) {
        if (dicipline == programm->getQry()->value(3).toString())
            continue;
    ...
    programm->selectFromProgrammsWideInfo(ui->title->text());
    while (programm->getQry()->next()) {
        if (participant == programm->getQry()->value(5).toString())
            continue;
...
void User::on_ok_clicked()
{
    ...
    date = QDate::currentDate();
    time = QTime::currentTime();
    programm->insertJournal(date.toString("yyyy-MM-dd"), time.toString("hh:mm:ss"), name, "Push Apply BTN");
}
